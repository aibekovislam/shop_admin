...
2. В карточке SKU (`Product variants`) заполните описание товара, категорию,
   бренд, минимум 3 фото, характеристики, наличие, количество и цену канала `MMarket Turan`.
...
Ключи можно писать как `цвет`, `color`, `память`, `memory`, `brand`,
`производитель` — при отправке в M-Market они нормализуются. Поля O!Market с
префиксом `omarket_` в M-Market не отправляются. Если поле `Бренд товара`
заполнено, оно автоматически уйдёт в `Производители`. Если заполнена
`Категория бренда` (`iPhone`, `AirPods`, `MacBook`), она автоматически уйдёт
в `Тип`.
